...
class CarDealer extends Company{
    ...
    function getContacts($sqldb)
	{
     ...
     {
         if ( $sqldb = mysql_connect( "sql310.epizy.com", "epiz_28094826", "0WJdsTQpbb", "epiz_28094826_db1"))
          {
             if ( mysql_select_db( "epiz_28094826_db1", $sqldb ))
              {
                   return $sqldb;
              }
              else
        ...
        $data .= "<li class=\"$cssClass\"><a href=\"" . $links['Contacts'] . "\" class=\"$cssClass\">Contacts</a></li>\n";
        $data .= "</ul>\n";
        $data .= "</div>\n";
        return $data;
     ...
     function displaySpecials()
     {
         ...
         $html .= "<tr><td colspan='4'><h1>Weekly Specials</h1></td></tr>";
